...
let canvas, context;
let buffer = ["Terminal Version 1.0", "(c) 2023 luckydye. All rights reserved.", ""];
let hiddenBuffer = [];
...
export default class Terminal extends HTMLElement {
  ...
  constructor() {
    ...
    const mouseDown = (e) => {
      ...
      if (e.button === 0) {
        ...
        setSelection(0, x + 4, y);
        setSelection(1, x + 4, y);
  ...
  drawCursor() {
    ...
    if (ts % 2 > 1) {
      ...
      context.fillStyle = FONT_COLOR;
      context.fillRect(pos[0], pos[1] - view[1] + 1, CURSOR_WIDTH, CURSOR_HEIGHT);
    }
  }
  ...
  getCursorPosition() {
    ...
    const y =
      BORDER_PADDING[1] + posY * CHAR_HEIGHT + posY * LINE_PADDING + CHAR_HEIGHT / 2 - CURSOR_HEIGHT / 2;
